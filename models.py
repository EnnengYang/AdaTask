...
class SegNetMtan(nn.Module):
    ...
    def shared_parameters(self) -> Iterator[nn.parameter.Parameter]:
        return (p for n, p in self.segnet.named_parameters() if "pred" not in n and 'coder_att' not in n)

    def task_specific_parameters(self) -> Iterator[nn.parameter.Parameter]:
        return (p for n, p in self.segnet.named_parameters() if "pred" in n or 'coder_att' in n)
...
class SegNetSplit(nn.Module):
    ...
    def forward(self, x):
        ...
        return ([t1_pred, t2_pred], (g_decoder[i][1]),)
